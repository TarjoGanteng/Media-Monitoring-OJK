...
def jalankan():
    with app.app_context():
        ...
        berita_all = (
            ...
            .order_by(order_case, Berita.tanggal.desc())
            .all()
        )
        logger.info(
            f"Mulai re-analisis sentimen {len(berita_all)} berita dengan panduan sentimen baru (OJK Perspective)..."
        )
        ...
        for i, b in enumerate(berita_all):
            try:
                ...
                if res:
                    ...
                    sukses += 1
                    logger.info(
                        f"[{i + 1}/{len(berita_all)}] '{b.judul[:40]}...' -> {b.sentimen}"
                    )
                else:
                    gagal += 1
                    logger.warning(
                        f"[{i + 1}/{len(berita_all)}] '{b.judul[:40]}...' -> Gagal Analisis"
                    )
