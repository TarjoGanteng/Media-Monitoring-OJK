...
def export_data():
    ...
    with app.app_context():
        ...
        print(f"Total: {len(data['users'])} User, {len(data['keywords'])} Keyword, {len(data['berita'])} Berita.")
        ...
        for kw_data in data.get("keywords", []):
            if not Keyword.query.filter_by(kata=kw_data["kata"]).first():
                kw = Keyword(
                    kata=kw_data["kata"],
                    aktif=kw_data["aktif"],
                    created_at=datetime.fromisoformat(kw_data["created_at"]) if kw_data.get("created_at") else datetime.utcnow()
                )
                db.session.add(kw)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Penggunaan:")
        print("  python seeder.py export  -> Menyimpan database Anda ke json untuk di push ke Git")
        print("  python seeder.py import  -> (Untuk teman Anda) Memasukkan json Git ke database lokalnya")
    else:
