...
def jalankan():
    with app.app_context():
        ...
        berita_negatif = Berita.query.filter_by(status='aktif', sentimen='Negatif').all()
            ...
            try:
                ...
                if res:
                    ...
                    if sentimen_baru != "Negatif":
                        berubah += 1
                        logger.info(f"[{i+1}/{len(berita_negatif)}] '{b.judul[:40]}...' BERUBAH: Negatif -> {sentimen_baru}")
                    else:
                        logger.info(f"[{i+1}/{len(berita_negatif)}] '{b.judul[:40]}...' TETAP: Negatif")
                    ...
                    gagal += 1
                    logger.warning(f"[{i+1}/{len(berita_negatif)}] '{b.judul[:40]}...' -> Gagal Analisis")
        ...
        logger.info(f"SELESAI! Berhasil: {sukses}, Gagal: {gagal}, Berubah dari Negatif: {berubah}")
